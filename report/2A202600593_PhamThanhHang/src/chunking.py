...
class RecursiveChunker:
    ...
    def _split(self, current_text: str, remaining_separators: list[str]) -> list[str]:
        ...
        sep = remaining_separators[0] if remaining_separators else ""
        next_separators = remaining_separators[1:] if remaining_separators else []
        ...
        for part in parts:
            if len(part) > self.chunk_size:
                ...
                added_len = len(part) if not current_chunk_parts else len(sep) + len(part)
                if current_len + added_len > self.chunk_size and current_chunk_parts:
                    final_chunks.append(sep.join(current_chunk_parts))
